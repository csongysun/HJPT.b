...
namespace Csys.Identity
{
    public class UserManager : IDisposable
    {
        ...
        public async Task<TaskResult> CreateAsync(SignUpModel model, string regIp)
        {
            var user = new User
            {
                Email = model.Email,
        ...
        public async Task<TaskResult> SignOutAsync(string id){
            //Todo: signout
        ...
        public async Task<TaskResult> UpdatePassKeyAsync(string id){
            var user = await _store.FindByIdAsync(id, CancellationToken);
            if(user == null)
                return EntityResult.EntityNotFound;
            user.PassKey = new Guid().ToString();
            UpdateUserAsync(user).RunSynchronously();
            return TaskResult.Success;
        }

        public async Task<User> FindUserByPassKey(string passKey){
            return await _store.FindByPassKeyAsync(passKey, CancellationToken);
        }
